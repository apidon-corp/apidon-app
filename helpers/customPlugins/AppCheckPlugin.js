...
const withCustomAppCheck = (config) => {
  return withAppDelegate(config, async (config) => {
    // Add import
    if (
      !config.modResults.contents.includes('#import "RNFBAppCheckModule.h"')
    ) {
    ...
    if (
      config.modResults.contents.includes(RNFI_CONFIGURE) &&
      !config.modResults.contents.includes(RNFI_CONFIGURE_WithDEBUGAPPCHECK)
    ) {
